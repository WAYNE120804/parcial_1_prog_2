...
public class SaleController {
    ...
    public ResponseEntity<ResponseDTO> getStoreByQuanty(int cant,String code) {
        int findsale = saleService.getTotalSales();
        while (findsale != 0) {
            return new ResponseEntity<>(new ResponseDTO(200, saleService.getTotalByStore(code)>cant,
                    null), HttpStatus.OK);

        }
        return new ResponseEntity<>(new ResponseDTO(409, "no hay ventas mayores a esa cantidad",
                null), HttpStatus.BAD_REQUEST);
    }
}
